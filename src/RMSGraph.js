  ...
  const contextMenuPlugin = {
    ...
    afterInit: (chart, arg, options) => {
      ...
      function handleContextMenu(e) {
        var menu = document.getElementById("contextMenu" + e.target.id);
        var dlg = document.getElementById("dlg" + e.target.id);
...
function RMSGraph(props) {
  ...
  const getData = () => {
    ...
    return {
      datasets: [
        {
          data: inputData.depth.map((x, index) => {
  ...
  const changeBackgroundHandler = (e, fields) => {
    ...
    if (e.native.ctrlKey && fields.length > 0) {
      ...
      if (newSelection.includes(curIndex)) {
        ...
        chartRef.current.config.data.datasets[0].backgroundColor[curIndex] =
          barColors[seriesData.color_type[curIndex]];
      } else {
  ...
  const hoverHandler = (e, fields) => {
    if (
      ...
      if (selection.length > 0) {
        ...
        if (ignoreFlag === true) labels.ignoreLabel = "Ignore Site";
        else labels.ignoreLabel = "Restore Site";
      } else {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            beforeBody: (items) => {
              return [
                `Depth: ${seriesData.depth[items[0].dataIndex]}`,
                `NRMS: ${seriesData.rms[items[0].dataIndex]}`,
              ];
            },
            title: () => "",
            label: () => "",
      ...
      scales: {
        x: {
          ...
          title: {
            color: "red",
            display:
              axisDirection.toUpperCase() === "HORIZONTAL"
                ? showDepthAxisLabel
                : showNRMSAxisLabel,
            text:
              axisDirection.toUpperCase() === "HORIZONTAL"
  ...
  const chartData = useMemo(
    () => getData(),
    [inputData, barColors, backgroundColor, selection]
  );
  ...
  const setIgnore = (e) => {
    ...
    if (selection.length > 0) {
      selection.forEach((item) => {
        ...
        chartRef.current.config.data.datasets[0].backgroundColor[item] =
          barColors[value];
      });
    } else {
  ...
  const setNormalize = () => {
    ...
    if (seriesData.color_type[currentItem] !== 1) {
      newState.color_type[currentItem] = 1;
      chartRef.current.config.data.datasets[0].backgroundColor[currentItem] =
        barColors[1];
    } else {
      chartRef.current.config.data.datasets[0].backgroundColor[currentItem] =
        barColors[0];
  ...
  // useEffect(() => {
  //   // console.log(index);
  //   var c = new ChartJS(chartRef.current.getContext("2d"), {
  //     plugin: [
  //       {
  //         id: "contextMenu",
  //         afterInit: (chart, arg, options) => {
  ...
  return (
    <>
      <div style={{ width: width, height: height }}>
        <Bar id={index} options={chartOption} data={chartData} ref={chartRef} />
        {/* <canvas id={index} ref={chartRef}></canvas> */}
      </div>
      <ul id={"contextMenu"+index} style={{ display: "none" }} ref={menuRef}>
        <li className="menu-item disabled" disabled>
          {inputData.depth[currentItem]}mm
        </li>
        <li className="menu-item" onClick={setIgnore}>
          {menuLabel.ignoreLabel}
        </li>
        <li className="menu-item" onClick={setNormalize}>
          {menuLabel.normalizeLabel}
        </li>
        <li className="menu-item" onClick={addMERNote}>
          Add MER Note
        </li>
        <li className="menu-item" onClick={merSite}>
          MER Site
        </li>
      </ul>
      <div
